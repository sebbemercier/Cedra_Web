...
export default function HeroSection() {
  ...
  return (
    <section className="relative w-full overflow-hidden flex flex-col items-center justify-start pt-16 pb-24 md:pt-32 md:pb-40 lg:min-h-212.5 bg-background selection:bg-cedra-500 selection:text-white">
      {/* Background Elements */}
      <div className="absolute inset-0 z-0 pointer-events-none">
        ...
        {/* Abstract Glows */}
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-250 h-150 bg-cedra-500/5 blur-[120px] rounded-full mix-blend-screen"></div>
        <div className="absolute bottom-0 right-0 w-200 h-150 bg-indigo-500/5 blur-[120px] rounded-full mix-blend-screen opacity-50"></div>
      </div>
      ...
      <div className="relative z-10 max-w-7xl w-full px-4 flex flex-col items-center text-center">
        ...
        >
          <StatBadge
            ...
            value="55K+"
            label={t.hero.products}
          />
          <StatBadge
            icon={<Clock size={14} />}
            value="24h"
            label={t.hero.dispatch}
            highlight
          />
          <StatBadge
            icon={<TrendingUp size={14} />}
            value="98%"
            label={t.hero.inStock}
          />
        </motion.div>
        ...
        >
          <h1 className="text-5xl sm:text-7xl md:text-8xl lg:text-9xl font-black text-white italic tracking-tighter uppercase leading-[0.85] mb-6 drop-shadow-2xl">
            {t.hero.title1}
            <br />
            <span className="text-transparent bg-clip-text bg-linear-to-r from-cedra-500 via-white to-cedra-600 animate-gradient pb-2 block">
              {t.hero.title2}
            </span>
          </h1>
          <p className="text-zinc-400 text-base sm:text-xl max-w-2xl mx-auto leading-relaxed px-4 font-medium tracking-wide">
            {t.hero.subtitle}{" "}
            <span className="text-cedra-500 font-bold">
              {t.hero.aiProcurement}
            </span>{" "}
            {t.hero.and}{" "}
            <span className="text-white font-bold underline decoration-cedra-500 underline-offset-4">
              {t.hero.delivery24h}
            </span>
          </p>
        </motion.div>
        ...
        >
          <div className="absolute -inset-1 bg-linear-to-r from-cedra-500/20 via-cedra-500/10 to-cedra-500/20 blur-2xl rounded-3xl opacity-0 group-focus-within:opacity-100 transition-all duration-700"></div>
...
function StatBadge({ icon, value, label, highlight }: StatBadgeProps) {
  return (
    <div
      className={`px-4 py-2 rounded-full border transition-all duration-300 ${highlight ? "bg-cedra-500/10 border-cedra-500/30 text-cedra-400 shadow-[0_0_20px_rgba(230,0,35,0.1)]" : "bg-white/5 border-white/5 text-zinc-400 hover:border-white/10 hover:bg-white/10"} backdrop-blur-md flex items-center gap-2 group cursor-default`}
    >
      <span
        className={`${highlight ? "text-cedra-500" : "text-zinc-500 group-hover:text-white"} transition-colors`}
      >
        {icon}
      </span>
...
function QuickAction({
  ...
  return (
    <motion.a
      ...
      className="group relative p-px rounded-2xl overflow-hidden transition-all duration-300 h-full"
    >
      ...
      <div className="relative h-full px-6 py-5 rounded-2xl bg-zinc-900/40 backdrop-blur-sm border border-white/5 group-hover:border-cedra-500/30 group-hover:bg-zinc-900/60 transition-all duration-500 flex flex-col items-start gap-4">
        ...
        <div className="text-left w-full">
          <div className="flex items-center justify-between gap-2 mb-1">
            <span className="text-sm font-black text-white uppercase italic tracking-tighter group-hover:text-cedra-400 transition-colors">
              {label}
            </span>
            {badge && (
              <Badge className="bg-cedra-500 text-white text-[9px] font-bold px-1.5 py-0 h-4 rounded-md border-none uppercase tracking-wider">
                {badge}
              </Badge>
            )}
          </div>
          <span className="text-xs text-zinc-500 group-hover:text-zinc-300 transition-colors font-medium leading-relaxed block">
            {description}
          </span>
        </div>

        <div className="mt-auto pt-2 flex items-center text-cedra-500 text-[10px] font-black uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-all duration-300 transform -translate-x-2.5 group-hover:translate-x-0">
