        ...
        >
          <div className="flex flex-col gap-4 h-full justify-between">
            <ul className="text-zinc-300 text-sm space-y-3">
              <BulletPoint icon={<Euro size={14} className="text-cedra-500" />}>
                {t.promo.vatExclusive}
              </BulletPoint>
              <BulletPoint
                icon={<Calendar size={14} className="text-cedra-500" />}
              >
                {t.promo.net30}
              </BulletPoint>
              <BulletPoint
                icon={<Cable size={14} className="text-cedra-500" />}
              >
                {t.promo.apiAccess}
              </BulletPoint>
              <BulletPoint
                icon={<Target size={14} className="text-cedra-500" />}
              >
                {t.promo.dedicatedManager}
              </BulletPoint>
            </ul>
            <div className="grid grid-cols-2 gap-2">
              <IconBox icon={<Package size={18} />} label={t.promo.inventory} />
              <IconBox icon={<BarChart3 size={18} />} label={t.promo.analytics} />
            </div>
        ...
        >
          <div className="h-full bg-gradient-to-br from-red-600/20 to-transparent rounded-xl flex flex-col items-center justify-center border border-red-500/20 relative overflow-hidden group/flash p-6">
            ...
            <div className="text-center relative z-10">
              ...
              <div className="text-[10px] text-zinc-400 flex items-center gap-1 justify-center">
                <Clock size={10} />
                {t.promo.expiresIn} 2j 14h
              </div>
            </div>
          </div>
        </PromoCard>
        ...
        >
          <div className="flex flex-col gap-3 h-full">
            ...
            <div className="text-center text-[10px] text-zinc-400">
              ...
              156 {t.promo.itemsAddedThisWeek}
            </div>
          </div>
        </PromoCard>
      </motion.div>
          ...
          <div className="h-full flex flex-col justify-center items-center text-center bg-zinc-900/50 border border-amber-500/10 rounded-xl p-6 relative overflow-hidden group/outlet">
            ...
            <div className="relative z-10">
              ...
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-amber-500/10 border border-amber-500/30 rounded-full">
                ...
                <span className="text-amber-400/60 text-xs">{t.promo.discount}</span>
              </div>
            </div>
          </div>
        </PromoCard>
...
function PromoCard({
  ...
  return (
    <motion.div
      ...
      className={`bg-gradient-to-br ${gradient} backdrop-blur-sm border border-white/5 rounded-2xl p-4 md:p-6 flex flex-col gap-4 min-h-[400px] hover:border-cedra-500/30 hover:shadow-2xl hover:shadow-cedra-500/10 transition-all group relative overflow-hidden`}
    >
      {/* Hover glow effect */}
      ...
      <div className="flex items-start justify-between relative z-10">
        ...
        {badge && (
          <Badge
            className={`${badgeStyles[badgeVariant]} text-[9px] px-2.5 py-0.5 font-bold uppercase tracking-wider`}
          >
            {badge}
          </Badge>
        )}
...
function ProductThumb({
  ...
  return (
    <div className="bg-black rounded-lg overflow-hidden border border-white/5 group/thumb cursor-pointer relative transform hover:scale-105 transition-all duration-300 aspect-square">
      <Image
        src={img}
        alt={name}
        fill
        sizes="100px"
        className="object-cover opacity-60 group-hover/thumb:opacity-100 group-hover/thumb:scale-110 transition-all duration-500"
        unoptimized
      />
      {newLabel && (
        <div className="absolute top-2 right-2 bg-blue-500 text-white text-[8px] font-black uppercase px-2 py-0.5 rounded-full shadow-lg z-10">
          {newText}
        </div>
      )}
      <div className="absolute inset-0 bg-gradient-to-t from-black via-black/50 to-transparent opacity-0 group-hover/thumb:opacity-100 transition-all duration-300 flex items-end p-3 z-10">
        <div className="transform translate-y-2 group-hover/thumb:translate-y-0 transition-transform duration-300">
          <span className="text-white text-[11px] font-bold block mb-0.5">
            {name}
          </span>
          <span className="text-cedra-400 text-[9px] uppercase tracking-wider flex items-center gap-1">
            {viewText} <ChevronRight size={10} />
          </span>
        </div>
      </div>
    </div>
...
function BulletPoint({
  ...
  return (
    <li className="flex items-center gap-2.5 group/bullet">
      <span className="group-hover/bullet:scale-110 transition-transform">
        {icon}
      </span>
