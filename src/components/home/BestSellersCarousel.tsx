...
export default function BestSellersCarousel() {
  ...
  return (
    <div className="relative">
      ...
      <div className="relative">
        <div className="absolute left-0 top-0 bottom-0 w-12 md:w-20 bg-gradient-to-r from-background to-transparent z-10 pointer-events-none"></div>
        <div className="absolute right-0 top-0 bottom-0 w-12 md:w-20 bg-gradient-to-l from-background to-transparent z-10 pointer-events-none"></div>
...
function ProductCardSkeleton() {
  return (
    <div className="min-w-[320px] bg-white/[0.02] border border-white/5 rounded-2xl p-5 space-y-4">
...
function ProductCard({ product, index, isHovered, onHover, onLeave, onClick, t }: { product: any; index: number; isHovered: boolean; onHover: () => void; onLeave: () => void; onClick: () => void; t: any }) {
  const badgeStyles = {
  ...
  const discountPercentage = product.oldPrice ? Math.round(((product.oldPrice - product.price) / product.oldPrice) * 100) : null;
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-4 right-4 z-20 flex flex-col gap-2 items-end">
        {product.badge && (
          <Badge className={cn("text-[9px] px-2.5 py-0.5 font-bold uppercase tracking-wider", badgeStyles[product.badgeVariant as keyof typeof badgeStyles] || "bg-zinc-700 text-white")}>
            {product.badge}
          </Badge>
        )}
        {discountPercentage && <Badge className="bg-red-500 text-white text-[9px] px-2 py-0.5 font-black">-{discountPercentage}%</Badge>}
      </div>
      <div className="aspect-[4/3] bg-black rounded-xl mb-4 relative overflow-hidden group/image">
        <Image src={product.image} alt={product.name} fill sizes="(max-width: 768px) 100vw, 320px" className="object-cover opacity-70 group-hover/image:opacity-100 group-hover/image:scale-110 transition-all duration-500" unoptimized />
        <AnimatePresence>
          {isHovered && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center gap-3">
      ...
      <div className="relative z-10">
        <h4 className="font-bold text-white mb-1 group-hover:text-cedra-400 transition-colors line-clamp-1">{product.name}</h4>
        <p className="text-zinc-400 text-xs mb-3 line-clamp-1">{product.series}</p>
        <div className="flex items-center gap-2 mb-3">
          <div className="flex items-center gap-1">
            {[...Array(5)].map((_, i) => (
              <Star key={i} size={12} className={cn("transition-colors", i < Math.floor(product.rating) ? "fill-amber-400 text-amber-400" : "text-zinc-700")} />
            ))}
          </div>
          <span className="text-zinc-400 text-[10px]">{product.rating} ({product.reviews} {t.bestSellers.reviews})</span>
        </div>
        <div className="flex items-center justify-between mb-4 p-2 bg-zinc-900/50 rounded-lg border border-white/5">
          <div className="flex items-center gap-2">
            <div className={cn("w-2 h-2 rounded-full", product.stock === t.bestSellers.inStock ? "bg-green-500 animate-pulse" : "bg-amber-500")}></div>
            <span className={cn("text-[10px] font-bold uppercase tracking-wider", product.stock === t.bestSellers.inStock ? "text-green-500" : "text-amber-500")}>{product.stock}</span>
          </div>
          <span className="text-zinc-500 text-[10px]">{product.stockCount} {t.bestSellers.units}</span>
        </div>
        <div className="flex items-end justify-between pt-4 border-t border-white/5">
          <div>
            <div className="text-zinc-400 text-[10px] uppercase tracking-wider mb-0.5">{t.bestSellers.price}</div>
            <div className="flex items-baseline gap-2">
              <span className="text-white font-mono font-bold text-xl">{product.price}€</span>
              {product.oldPrice && <span className="text-zinc-500 font-mono text-sm line-through">{product.oldPrice}€</span>}
            </div>
          </div>
