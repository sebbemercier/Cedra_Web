    ...
    setTimeout(() => {
      setMessages((prev) => [
        ...prev,
        {
          role: "assistant",
          content:
            "Analyse ScyllaDB terminée. Stock disponible pour l'ID demandé. Remise Pro de 15% applicable sur ce volume.",
        },
      ]);
      setIsLoading(false);
    }, 1000);
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-8 right-8 w-95 h-150 z-50 flex flex-col rounded-[2.5rem] overflow-hidden bg-zinc-900/40 backdrop-blur-3xl border border-white/10 shadow-[0_25px_50px_-12px_rgba(0,0,0,0.5)]"
          >
            {/* Header Minimaliste */}
            <div className="p-6 pb-4 flex items-center justify-between border-b border-white/5">
              ...
              <button
                ...
                className="w-8 h-8 rounded-full hover:bg-white/5 flex items-center justify-center text-white/20 hover:text-white transition-all"
              >
                <X size={18} />
              </button>
            </div>
              ...
              >
                <div className="space-y-8 pb-4">
                  {messages.map((m, i) => (
                    ...
                    >
                      <div
                        className={`max-w-[85%] space-y-1 ${m.role === "user" ? "text-right" : "text-left"}`}
                      >
                        <div
                          className={`px-4 py-3 rounded-2xl text-[13px] font-medium leading-relaxed ${
                            m.role === "user"
                              ? "bg-white text-black rounded-tr-sm shadow-xl"
                              : "bg-white/3 text-zinc-300 border border-white/5 rounded-tl-sm"
                          }`}
                        >
                          {m.content}
                        </div>
            ...
            <div className="p-6 bg-black/20">
              <div className="relative group">
                <input
                  ...
                  className="w-full bg-white/3 border border-white/10 rounded-2xl py-4 pl-5 pr-14 text-white text-sm focus:outline-none focus:border-red-600/50 transition-all placeholder:text-white/10 font-medium"
                />
                <button
                  onClick={handleSend}
